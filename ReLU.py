...
all_results = []
def ReLU_simulation(ReLU_name,ReLU_title, InputValueIn):
    global all_results  
    all_results.clear()
    for ii in range(InputValueIn.shape[0]):
        ...
        with open(file_path, 'w') as cir_file:
            ...
            for index in range(len(InputValue_flattened_array)):
                cir_file.write(f"V{index + 1} QRevOut{index + 1} 0 {InputValue_flattened_array[index]}\n")
        ...
        circuit_file = os.path.join(output_directory, f'{ReLU_name}{ii}.cir')
    ...
    result_matrix = result_matrix.reshape(
         (InputValueIn.shape[0],InputValueIn.shape[1],InputValueIn.shape[2]))
