...
def Conv_simulation(Conv_name,Conv_title, InputValueIn, ConvValuePIn, ConvValueNIn, ConvBiasVoltage,ConvPadding, ConvStride, ConvBiasPIn, ConvBiasNIn):
    all_results = []
    if ConvBiasPIn is not None:
        for ii in range(ConvValuePIn.shape[0]):
            ConvBiasP = ConvBiasPIn[ii, :]
            ConvBiasN = ConvBiasNIn[ii, :]
            for jj in range(InputValueIn.shape[0]):
                InputValue = InputValueIn[jj, :, :]
                ConvValueP=ConvValuePIn[ii,jj,:,:]
                ConvValueN=ConvValueNIn[ii,jj,:,:]
                ...
                InputExtended[ConvPadding:ConvPadding + InputNumRows, ConvPadding:ConvPadding + InputNumColumns] = InputValue
                ...
                file_name = f'{Conv_name}{ii}{jj}.cir'
                ...
                with open(file_path, 'w') as cir_file:
                    ...
                    for position, info in result_dict.items():
                        ...
                        if(formatted_value!='999999936.0000000000'):
                            cir_file.write(f"NCQ{index} CQInput{info['col']} CQVMMOut{info['row']} HP wic={formatted_value}\n")
                        index += 1
                    cir_file.write("\n")
                    for index, value in enumerate(ConvBiasP):
                        formatted_value = f"{value:.10f}"
                        if(formatted_value!='999999936.0000000000'):
                            formatted_value = float(formatted_value) / ConvValuePIn.shape[1]
                            cir_file.write(f"NCQB{index + 1} b CQVMMOut{index + 1} HP wic={formatted_value}\n")
                    cir_file.write("\n")
                    ...
                    for position, info in result_dict_n.items():
                        formatted_value = f"{info['value']:.10f}"
                        if(formatted_value!='999999936.0000000000'):
                            cir_file.write(f"NCQN{index} CWInput{info['col']} CQVMMOut{info['row']} HP wic={formatted_value}\n")
                        index = index + 1
                    cir_file.write("\n")
                    for index, value in enumerate(ConvBiasN):
                        formatted_value = f"{value:.10f}"
                        if(formatted_value!='999999936.0000000000'):
                            formatted_value = float(formatted_value) / ConvValuePIn.shape[1]
                            cir_file.write(f"NCQBN{index + 1} nb CQVMMOut{index + 1} HP wic={formatted_value}\n")
                    cir_file.write("\n")
                ...
                circuit_file = os.path.join(output_directory, f'{Conv_name}{ii}{jj}.cir')
                ...
                n = InputValueIn.shape[0]
                result_matrix = np.array([np.sum(matrix[i * n : (i + 1) * n, :], axis=0) for i in range(len(matrix) // n)])
        ...
        for ii in range(ConvValuePIn.shape[0]):
            for jj in range(InputValueIn.shape[0]):
                ...
                result_matrix = np.array(
                    [np.sum(matrix[i * n: (i + 1) * n, :], axis=0) for i in range(len(matrix) // n)])
