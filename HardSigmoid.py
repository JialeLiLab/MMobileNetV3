...
all_results = []
def HardSigmoid_simulation(hardsigmoid_name,hardsigmoid_title, InputValueIn):
    global all_results 
    all_results.clear()
    ...
    for ii in range(FilesNumber):
        file_name = f'{hardsigmoid_name}{ii}.cir'
        ...
        with open(file_path, 'w') as cir_file:
            cir_file.write(hardsigmoid_title)
            cir_file.write("\n")
            ...
            cir_file.write("op\n")
            for index in range(len(InputValue_flattened_array)):
                cir_file.write(f"print OutdivP{index + 1}\n")
            cir_file.write(".endc\n")
            cir_file.write(".END")
        ngspice_command = 'ngspice'
        circuit_file = os.path.join(output_directory, f'{hardsigmoid_name}{ii}.cir')
        command = [ngspice_command, '-b', circuit_file]
        result = subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        print(result)
        current_values = []
        for line in result.stdout.split('\n'):
            if line.startswith('outdivp'):
                parts = line.split('=')
    ...
    result_matrix = result_matrix.reshape((InputShape0,InputShape1,InputShape2))
